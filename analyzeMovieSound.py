...
def getMusicSegmentsFromFile(inputFile):
    ...
    flagsInd, classNames, acc, CM = aS.mtFileClassification(
        inputFile, modelName, modelType, plotResults=False, gtFile="")
    segs, classes = aS.flags2segs(flagsInd, mtStep)

    for i, s in enumerate(segs):
        if (classNames[int(classes[i])] == "Music") and (s[1] - s[0] >= minDuration):
            strOut = "{0:s}{1:.3f}-{2:.3f}.wav".format(
                dirOutput + os.sep, s[0], s[1])
...
def main(argv):
    ...
    if argv[1] == "--file":
        ...
        classifyFolderWrapper(
            argv[2][0:-4] + "_musicSegments", "svm", "data/svmMusicGenre8", True)
        ...
        with open(csvFile, 'rb') as csvfile:
            ...
            for i in range(f.shape[0]):
                for j in range(f.shape[0]):
                    Sim[i, j] = scipy.spatial.distance.cdist(numpy.reshape(
                        f[i, :], (f.shape[1], 1)).T, numpy.reshape(f[j, :], (f.shape[1], 1)).T, 'cosine')
        ...
        for i, w in enumerate(wavFilesList):
            [flagsInd, classesAll, acc, CM] = aS.mtFileClassification(
                w, "data/svmMovies8classes", "svm", False, '')
