...
class kNN:
    ...
    def classify(self, testSample):
        ...
        YDist = (distance.cdist(self.X, testSample.reshape(
            1, testSample.shape[0]), 'euclidean')).T
        iSort = numpy.argsort(YDist)
        P = numpy.zeros((nClasses,))
        for i in range(nClasses):
            P[i] = numpy.nonzero(self.Y[iSort[0][0:self.k]] == i)[
                0].shape[0] / float(self.k)
...
def featureAndTrain(listOfDirs, mtWin, mtStep, stWin, stStep, classifierType, modelName, computeBEAT=False, perTrain=0.90):
    ...
    [features, classNames, _] = aF.dirsWavFeatureExtraction(
        listOfDirs, mtWin, mtStep, stWin, stStep, computeBEAT=computeBEAT)
    ...
    if classifierType == "knn":
        ...
        print((modelName + " saved"))
            ...
            or classifierType == "randomforest" \
            or classifierType == "gradientboosting" \
...
def featureAndTrainRegression(dirName, mtWin, mtStep, stWin, stStep, modelType, modelName, computeBEAT=False):
    ...
    [features, _, fileNames] = aF.dirsWavFeatureExtraction(
        [dirName], mtWin, mtStep, stWin, stStep, computeBEAT=computeBEAT)
    features = features[0]
    ...
    for c in CSVs:                                                            # for each CSV
        ...
        # # read filenames, map to "fileNames" and append respective values in
        # the regressionLabels
        curRegressionLabels = []
        featuresTemp = []
        # open the csv file that contains the current target value's
        # annotations
        with open(c, 'rb') as csvfile:
            CSVreader = csv.reader(csvfile, delimiter=',', quotechar='|')
            for row in CSVreader:
                # if the current row contains two fields (filename, target
                # value)
                if len(row) == 2:
                    # ... and if the current filename exists in the list of filenames
        ...
        # curRegressionLabels is the list of values for the current regression
        # problem
...
#        modelParams = numpy.array([1, 3, 5, 7, 9, 11, 13, 15]);
    ...
    for iRegression, r in enumerate(regressionNames):
        ...
        if modelType == "svm":
            Classifier, _ = trainSVMregression(
                featuresNorm[0], regressionLabels[iRegression], bestParam)
        ...
        if modelType == "svm" or modelType == "svm_rbf" or modelType == "randomforest":
            # save to file
            with open(modelName + "_" + r, 'wb') as fid:
                cPickle.dump(Classifier, fid)
        ...
        for e in range(nExp):
            ...
            for c1 in range(nClasses):
                ...
                for ss in range(nTestSamples):
                    [Results[ss], _] = classifierWrapper(
                        Classifier, ClassifierName, featuresTest[c1][ss])
    ...
    for Ci, C in enumerate(Params):                # for each param value
        ...
        for e in range(nExp):
            ...
            featuresTest = [featuresNorm[randperm[i + nTrain]]
                            for i in range(nSamples - nTrain)]
            labelsTrain = [labels[randperm[i]] for i in range(nTrain)]
            labelsTest = [labels[randperm[i + nTrain]]
                          for i in range(nSamples - nTrain)]

            # train multi-class svms:
            featuresTrain = numpy.matrix(featuresTrain)
            if MethodName == "svm":
                [Classifier, trainError] = trainSVMregression(
                    featuresTrain, labelsTrain, C)
            elif MethodName == "svm_rbf":
                [Classifier, trainError] = trainSVMregression_rbf(
                    featuresTrain, labelsTrain, C)
            elif MethodName == "randomforest":
                [Classifier, trainError] = trainRandomForestRegression(
                    featuresTrain, labelsTrain, C)
            ErrorTest = []
            ErrorTestBaseline = []
            for itest, fTest in enumerate(featuresTest):
                ...
                ErrorTest.append(
                    (R - labelsTest[itest]) * (R - labelsTest[itest]))
                ErrorTestBaseline.append(
                    (Rbaseline - labelsTest[itest]) * (Rbaseline - labelsTest[itest]))
...
def fileClassification(inputFile, modelName, modelType):
    ...
    if (modelType) == 'svm' or (modelType == 'svm_rbf'):
        [Classifier, MEAN, STD, classNames, mtWin, mtStep,
            stWin, stStep, computeBEAT] = loadSVModel(modelName)
    elif modelType == 'knn':
        [Classifier, MEAN, STD, classNames, mtWin, mtStep,
            stWin, stStep, computeBEAT] = loadKNNModel(modelName)
    elif modelType == 'randomforest':
        [Classifier, MEAN, STD, classNames, mtWin, mtStep, stWin,
            stStep, computeBEAT] = loadRandomForestModel(modelName)
    elif modelType == 'gradientboosting':
        [Classifier, MEAN, STD, classNames, mtWin, mtStep, stWin,
            stStep, computeBEAT] = loadGradientBoostingModel(modelName)
    elif modelType == 'extratrees':
        [Classifier, MEAN, STD, classNames, mtWin, mtStep, stWin,
            stStep, computeBEAT] = loadExtraTreesModel(modelName)
    ...
    [Result, P] = classifierWrapper(
        Classifier, modelType, curFV)    # classification
...
def fileRegression(inputFile, modelName, modelType):
    ...
    if modelType == 'svm' or modelType == "svm_rbf":
        [_, _, _, mtWin, mtStep, stWin, stStep, computeBEAT] = loadSVModel(
            regressionModels[0], True)
    elif modelType == 'randomforest':
        [_, _, _, mtWin, mtStep, stWin, stStep, computeBEAT] = loadRandomForestModel(
            regressionModels[0], True)
    ...
    for ir, r in enumerate(regressionModels):
        if not os.path.isfile(r):
            ...
            return (-1, -1, -1)
        if modelType == 'svm' or modelType == "svm_rbf":
            [Model, MEAN, STD, mtWin, mtStep, stWin,
                stStep, computeBEAT] = loadSVModel(r, True)
        elif modelType == 'randomforest':
...
def trainSpeakerModelsScript():
    ...
    listOfDirs = [os.path.join(dirName, name) for name in os.listdir(
        dirName) if os.path.isdir(os.path.join(dirName, name))]
    featureAndTrain(listOfDirs, mtWin, mtStep, stWin, stStep, "knn",
                    "data/knnSpeakerAll", computeBEAT=False, perTrain=0.50)

    dirName = "DIARIZATION_ALL/female_male"
    listOfDirs = [os.path.join(dirName, name) for name in os.listdir(
        dirName) if os.path.isdir(os.path.join(dirName, name))]
    featureAndTrain(listOfDirs, mtWin, mtStep, stWin, stStep, "knn",
                    "data/knnSpeakerFemaleMale", computeBEAT=False, perTrain=0.50)
