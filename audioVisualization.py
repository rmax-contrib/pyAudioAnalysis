...
def generateColorMap():
    ...
    for i in range(Map.shape[0]):
        rgb = (int(255 * Map[i][0]), int(255 *
                                         Map[i][1]), int(255 * Map[i][2]))
...
def levenshtein(str1, s2):
    ...
    for i in range(0, N2):
        for j in range(0, N1):
            if str1[j] == s2[i]:
                stringRange[i + 1][j + 1] = min(stringRange[i + 1][j] + 1, stringRange[
                                                i][j + 1] + 1, stringRange[i][j])
            else:
                stringRange[i + 1][j + 1] = min(stringRange[i + 1][j] + 1, stringRange[
                                                i][j + 1] + 1, stringRange[i][j] + 1)
...
def textListToColors(names):
    ...
    for i in range(len(names)):
        for j in range(len(names)):
            Dnames[i, j] = 1 - 2.0 * \
                levenshtein(names[i], names[j]) / \
                float(len(names[i] + names[j]))

    # STEP B: pca dimanesionality reduction to a single-dimension (from the
    # distance space)
...
#        M = np.mean(a[0:int(SM2.shape[1]/3+1)])
    ...
    shutil.copyfile("data/similarities.html", dirChordial +
                    os.sep + "similarities.html")
...
def visualizeFeaturesFolder(folder, dimReductionMethod, priorKnowledge="none"):
    ...
    if dimReductionMethod == "pca":
        ...
        namesCategoryToVisualize = [ntpath.basename(w).replace(
            '.wav', '').split(" --- ")[0] for w in wavFilesList]
        namesToVisualize = [ntpath.basename(w).replace(
            '.wav', '') for w in wavFilesList]

        (F, MEAN, STD) = aT.normalizeFeatures([allMtFeatures])
        F = np.concatenate(F)

        # check that the new PCA dimension is at most equal to the number of
        # samples
        K1 = 2
        K2 = 10
        if K1 > F.shape[0]:
        ...
        # long-term statistics cannot be applied in this context (LDA needs
        # mid-term features)
        ...
        if priorKnowledge == "artist":
            ...
            # for each unique artist name:
            for i, uname in enumerate(uNamesCategoryToVisualize):
                indicesUCategories = [j for j, x in enumerate(
                    namesCategoryToVisualize) if x == uname]
        ...
        # uLabels must have as many labels as the number of wavFilesList
        # elements
    ...
    for i in range(finalDims.shape[0]):
        plt.text(finalDims[i, 0], finalDims[i, 1], ntpath.basename(wavFilesList[i].replace(
            '.wav', '')), horizontalalignment='center', verticalalignment='center', fontsize=10)
    ...
    chordialDiagram("visualization", SM, 0.50,
                    namesToVisualize, namesCategoryToVisualize)

    SM = 1.0 - distance.squareform(distance.pdist(F, 'cosine'))
    for i in range(SM.shape[0]):
        SM[i, i] = 0.0
    chordialDiagram("visualizationInitial", SM, 0.50,
                    namesToVisualize, namesCategoryToVisualize)
    ...
    SMgroup = 1.0 - \
        distance.squareform(distance.pdist(finalDimsGroup, 'cosine'))
